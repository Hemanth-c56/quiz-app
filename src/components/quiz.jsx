...
export default function Quiz() {
  ...
  const quizDuration = 2 * 60 * 1000; // 2 minutes in milliseconds
...
// -----------------------------------------------------------------------------------
  ...
  function toAnalysis(){
    buttonAudio.play()
    var minnow = Math.floor(timer / 60000)
    var secondsnow = ((timer % 60000) / 1000 < 10 ? '0' : '')+(Math.floor((timer % 60000) / 1000))
    navigate("/analysis",{state :{score:score,qattempted:Qattempted.length,minute:minnow,second:secondsnow}})
  }
    return (
        <div className='quiz-body'>
            <div className="quiz-container">
              <nav className='Nav-bar'>
                <div className='timer-div'><div>DO YOUR BEST</div> <div>{Math.floor(timer / 60000)}:{(timer % 60000) / 1000 < 10 ? '0' : ''}{Math.floor((timer % 60000) / 1000)} min</div></div>
